...
### 盘前新闻 — 操作演示（点击按钮一键生成：6 大财经平台站点定向并发搜索 + 美股夜盘专项 + 知识星球素材聚合 → DeepSeek 云端推理 → 结构化表格输出，全程实时进度可见）
![盘前新闻功能演示动画](screenshots/盘前新闻功能演示动画2.gif)

### 盘前新闻 — 输出效果图（三段式结构化报告：平台热点总结 / 美股科技股盘前盘中表现，均为表格渲染）
![盘前新闻输出结果展示-表格](screenshots/盘前新闻输出结果展示1.png)

### 盘前新闻 — 输出效果图（推理分析与预测：利好/利空 A 股概念个股 + 风险提示，个股均标注「需自选验证」）
![盘前新闻输出结果展示-推理预测](screenshots/盘前新闻输出结果展示2.png)
