...
### 盘前研报热度分析 — 操作演示（知识星球 Playwright 抓取 + Ollama qwen3:8b 本地动态分批抽取）
![盘前研报热度操作演示](screenshots/4.盘前研报热度演示动画.gif)

### 盘前研报热度分析 — 输出效果图（前端表格渲染，股票名/情绪/研报数/行业/摘要）
![盘前研报热度输出效果图](screenshots/5.盘前研报热度输出效果.png)

### 盘前新闻 — 操作演示（点击按钮一键生成：6 大财经平台站点定向并发搜索 + 美股 + 知识星球素材聚合 → DeepSeek 云端推理 → 结构化表格输出，全程实时进度可见）
![盘前新闻功能演示动画](screenshots/1.盘前新闻功能演示动画.gif)

### 盘前新闻 — 输出效果图（三段式结构化报告：平台热点总结 / 美股科技股盘前盘中表现，均为表格渲染）
![盘前新闻输出结果展示-表格](screenshots/2.盘前新闻输出效果1.png)

### 盘前新闻 — 输出效果图（推理分析与预测：利好/利空 A 股概念个股 + 风险提示，个股均标注「需自选验证」）
![盘前新闻输出结果展示-推理预测](screenshots/3.盘前新闻输出效果2.png)

### 复盘预测 — 操作演示（盘前研报热度 + 盘前新闻双源并行 → DeepSeek 综合分析与大盘预测；3 小时内存档秒级复用）
![复盘预测功能演示动画](screenshots/6.复盘预测功能演示动画.gif)

### 复盘预测 — 输出效果图（多维综合分析，自动存档 output/Market_Recap_Outlook）
