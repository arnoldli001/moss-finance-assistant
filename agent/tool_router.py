...
# ===== CancellationToken: 跨层级取消联动检查点（工具路由/执行前主动检查）=====
...
# 预设索引（项目实际工具）
# 说明：子 agent 在 deepagents 中被打包为统一名为 task 的 StructuredTool，
# 但通过参数 subagent_type 区分子类型。我们把 3 个子类型也在菜单中单列，
# 只要任一命中，就把 task 工具整体披露。
PRESET_INDEX: Dict[str, ToolIndexEntry] = {
    ...
    "read_file_content": ToolIndexEntry(
        ...
        category="文件读取",
        summary="读取用户上传文件（支持 md/docx/pdf/xlsx/xls/txt）",
        trigger_keywords=["读取文件", "上传的", "附件", "文件内容", "读excel", "读pdf", "读word", "解析",
                          ".pdf", ".xlsx", ".xls", ".docx", ".doc", ".md", ".txt",
                          "我给你的", "上传", "打开文件", "查看文件内容", "导入文件"],
    ),
    "search_zsxq_by_stock": ToolIndexEntry(
        tool_id="search_zsxq_by_stock",
        category="知识星球搜索",
        # 注意：summary 中显式声明"仅单只股票 + 用户明确提到知识星球"，
        # 避免盘前新闻/盘前策略等批量多股任务误触发本工具的级联动作。
        summary="仅在用户明确询问'知识星球/小作文/圈子'中某只股票讨论时调用；批量分析任务禁用",
        # trigger_keywords 收紧：只保留明确指向"知识星球"语义的词，
        # 移除"研报"（太宽泛，所有盘前新闻任务都会命中）和"热度"
        # （"盘前小作文热度"按钮专用，已由 /api/zsxq-analysis 独立入口处理，
        # 不应让本工具在"盘前新闻"按钮的 prompt 中被自动披露）。
