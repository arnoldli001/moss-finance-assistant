...
class StockCodeFormatRule(ValidationRule):
    ...
    实际项目应接 tushare/akshare 验证代码是否存在，这里做基本格式校验。
    """
    name = "stock_code_format"
    default_severity = "block"

    # A 股代码规则
    _A_STOCK_PATTERN = re.compile(r"\b(60\d{4}|00\d{4}|30\d{4}|688\d{3}|8\d{5})\b")
    # 港股 5 位
    _HK_STOCK_PATTERN = re.compile(r"\b0\d{4}\b")
    ...
    async def check(self, ctx: ValidationContext) -> List[Violation]:
        ...
        # 找出所有疑似股票代码
        a_codes = self._A_STOCK_PATTERN.findall(output)
        # 这里仅做格式校验；实际生产应查询数据库验证代码是否真实存在
        # 例如：select code from stock_basic where code in (...)
        for code in a_codes:
            # 简单规则：6 开头是上交所主板（60xxxx）；0 是深交所主板；30 是创业板；688 是科创板；8 是北交所
            if not (code.startswith("60") or code.startswith("00") or
                    code.startswith("30") or code.startswith("688") or
                    code.startswith("8")):
                violations.append(Violation(
                    rule_name=self.name,
                    severity="warn",  # 格式异常降级为 warn，避免误拦
                    message=f"股票代码 {code} 格式异常，可能不存在",
                    evidence=code,
                    fix_hint="请确认代码是否来自检索结果，未检索到的不要写",
                ))
        return violations
