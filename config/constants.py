...
# 点击"当前星球"/"最新"后的等待秒数
ZSXQ_SEARCH_FILTER_BTN_AFTER_CLICK_WAIT_SEC: int = 1

# 等待搜索结果组件出现的超时（毫秒）
ZSXQ_SEARCH_RESULTS_SELECTOR_TIMEOUT_MS: float = 1000

# 搜索结果备选选择器等待超时（毫秒）
ZSXQ_SEARCH_RESULTS_FALLBACK_TIMEOUT_MS: float = 1000

# 滚动加载更多主题后的等待秒数
ZSXQ_SEARCH_SCROLL_MORE_WAIT_SEC: int = 1

# 搜索结果主题卡片 scroll_into_view 超时（毫秒）
ZSXQ_SEARCH_CARD_SCROLL_INTO_VIEW_TIMEOUT_MS: float = 1000

# 搜索结果卡片滚动进入视口后的额外等待秒数
ZSXQ_SEARCH_CARD_AFTER_SCROLL_WAIT_SEC: float = 0.3

# 点击搜索结果卡片的超时（毫秒）
ZSXQ_SEARCH_CARD_CLICK_TIMEOUT_MS: float = 1000

# 搜索结果卡片内部 fallback 点击超时（毫秒）
ZSXQ_SEARCH_CARD_INNER_CLICK_TIMEOUT_MS: float = 1000

# 进入主题详情视图后的等待渲染秒数
ZSXQ_SEARCH_DETAIL_AFTER_OPEN_WAIT_SEC: int = 1

# 详情正文选择器：元素可见等待超时（毫秒）
ZSXQ_SEARCH_DETAIL_TEXT_VISIBLE_TIMEOUT_MS: float = 1000

# 详情正文选择器：inner_text 提取超时（毫秒）
ZSXQ_SEARCH_DETAIL_TEXT_INNER_TIMEOUT_MS: float = 1000
...
ZSXQ_SEARCH_DETAIL_AUTHOR_INNER_TIMEOUT_MS: float = 1000
