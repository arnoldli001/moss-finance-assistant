    ...
    try:
        ...
        with connect(**config) as  conn:
            with conn.cursor() as cursor:
                ...
                # 可用的表有：表1,表2,表3....
                # [表1,表2,表3]
                table_names: list = [str(tuple(table)[0]) for table in tables]
...
def execute_sql_query(query)->str:
    ...
    # ===== SQL 注入防护 =====
    # 1. 禁止多语句执行（分号分隔的多条 SQL）
    stripped = query.strip()
    if stripped.endswith(";"):
        stripped = stripped[:-1].strip()
    if ";" in stripped:
        return "安全拒绝：禁止执行多条 SQL 语句（检测到分号）"
    # 2. 只允许 SELECT / WITH 开头的只读查询
    sql_upper = stripped.upper()
    if not sql_upper.startswith("SELECT") and not sql_upper.startswith("WITH"):
        return f"安全拒绝：仅允许 SELECT 查询，禁止 DDL/DML 操作"
    # 3. 禁止危险关键词（防止 SQL 注入中夹带 DROP/DELETE 等）
    import re
    dangerous_keywords = ["DROP", "DELETE", "TRUNCATE", "ALTER", "INSERT",
                          "UPDATE", "CREATE", "GRANT", "REVOKE", "EXEC",
                          "MERGE", "REPLACE", "LOAD_FILE", "OUTFILE", "DUMPFILE"]
    for kw in dangerous_keywords:
        if re.search(rf"\b{kw}\b", sql_upper):
            return f"安全拒绝：SQL 中包含禁止的关键词 '{kw}'，仅允许只读查询"
...
if __name__ == "__main__":
    print(execute_sql_query.invoke({"query": "SELECT * FROM `drugs` dgs join sales_records srd on dgs.drug_id = srd.drug_id"}))
