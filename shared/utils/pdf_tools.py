...
from utils.word_converter import convert_md_to_pdf_via_word, _rel_display  # noqa: F401  (对外展示复用)
...
def convert_md_to_pdf(
    ...
    try:
        ...
        # 2. 检查源文件（对外提示使用相对路径或文件名，不暴露磁盘）
        if not md_abs_path.exists():
            display = Path(md_path).name if Path(md_path).name else str(md_path)
            return f"错误：文件不存在 {display}"
