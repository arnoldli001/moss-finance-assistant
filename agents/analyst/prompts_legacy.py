...
# 项目的根地址：向祖先目录查找 AGENTS.md（保守兼容：新旧目录位置都能定位项目根）
def _find_project_root(start: Path) -> Path:
    cur = start.resolve()
    for _ in range(8):
        if (cur / "AGENTS.md").exists() or (cur / ".git").exists() or (cur / "requirements.txt").exists():
            return cur
        if not cur.parent or cur.parent == cur:
            break
        cur = cur.parent
    return start.parents[1]  # 兜底：保留原 parents[1] 行为

project_root_path = _find_project_root(Path(__file__))
yaml_file_path = project_root_path / "prompt" / "prompts.yml"

# 兼容：若 prompt/ 目录已迁移（重构阶段），尝试 shared/config/prompts.yml 第二路径
if not yaml_file_path.exists():
    alt = project_root_path / "shared" / "config" / "prompts.yml"
    if alt.exists():
        yaml_file_path = alt
