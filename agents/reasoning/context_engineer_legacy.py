...
class ContextEngineer:
    ...
    # ------------------------------------------------------------------
    # 股票代码 + 名称抽取（集成 StockMatcher，兼顾有效性校验与名称识别）
    # ------------------------------------------------------------------
    def _extract_stock_codes(self, text: str) -> List[str]:
        ...
        来源优先级：
          1) 先调用 StockMatcher.extract_from_text 抽取"名称+代码"复合识别，
             有效避免 6 位数字误识别为日期、编号。
          2) 再用 _STOCK_CODE_REGEX 兜底扫后缀格式（0700.HK / AAPL.US 等），
             这部分不在 A 股清单内时按原样保留（保留港美股后缀格式）。
        去重并保持出现顺序。
        """
        if not text:
            return []
        ...
        # Step A: 集成 StockMatcher，仅保留清单内的 A 股代码
        for s in _matcher_extract_stocks(text):
            if s.code not in seen:
                seen.add(s.code)
                codes.append(s.code)

        # Step B: 正则兜底（含 HK/US/SH/SZ 后缀格式；纯 6 位仅保留清单内有效代码）
        for m in _STOCK_CODE_REGEX.findall(text):
            code_orig = m
            code_norm = code_orig.upper()
            # 纯 6 位数字：用 StockMatcher 校验是否在清单中，避免 202501 之类日期误识别
            if len(code_norm) == 6 and code_norm.isdigit():
                if not _matcher_is_valid_code(code_norm):
                    continue
